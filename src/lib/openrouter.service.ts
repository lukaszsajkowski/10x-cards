...
export class OpenRouterService {
   ...
   */
  private buildRequestPayload<T>(params: ChatCompletionParams<T>): OpenRouterRequestPayload {
   ...
   */
  private buildResponseFormat<T>(responseSchema: ResponseSchema<T>): OpenRouterResponseFormat {
   ...
   */
  private async executeRequest(payload: OpenRouterRequestPayload): Promise<OpenRouterRawResponse> {
   ...
   */
  private parseResponse<T>(rawResponse: OpenRouterRawResponse, schema: z.ZodType<T>): T {
    const content = rawResponse.choices?.[0]?.message?.content;

    if (!content) {
      throw new OpenRouterResponseError("Empty response content from API", "EMPTY_RESPONSE");
    }
    ...
    try {
      ...
      throw new OpenRouterResponseError("Failed to parse JSON response", "INVALID_JSON");
    }

    const validated = schema.safeParse(parsed);

    if (!validated.success) {
      throw new OpenRouterResponseError(`Response validation failed: ${validated.error.message}`, "VALIDATION_FAILED");
    }
  ...
  private async handleHttpError(response: Response): Promise<OpenRouterError> {
    ...
    switch (response.status) {
      ...
      case 429: {
        ...
        return new OpenRouterRateLimitError("Rate limit exceeded", "RATE_LIMITED", retryAfter);
      }
      case 500:
      case 502:
      case 503:
        return new OpenRouterServerError(`Server error: ${response.status}`, "SERVER_ERROR");
      default:
        return new OpenRouterError(`HTTP error: ${response.status} - ${errorMessage}`, "UNKNOWN_ERROR");
    }
  }
...
if (!apiKey) {
  console.warn("OPENROUTER_API_KEY is not set. OpenRouter service will not be available.");
}

export const openRouterService = apiKey ? new OpenRouterService({ apiKey }) : null;
