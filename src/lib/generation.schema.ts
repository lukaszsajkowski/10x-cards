...
export const createGenerationCommandSchema = z.object({
  source_text: z
    .string()
    .min(SOURCE_TEXT_MIN_LENGTH, `source_text must be at least ${SOURCE_TEXT_MIN_LENGTH} characters long`)
    .max(SOURCE_TEXT_MAX_LENGTH, `source_text must be at most ${SOURCE_TEXT_MAX_LENGTH} characters long`),
});

export type CreateGenerationCommandInput = z.infer<typeof createGenerationCommandSchema>;
