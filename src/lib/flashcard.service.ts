...
export class FlashcardService {
  async createFlashcards(params: CreateFlashcardsParams): Promise<CreateFlashcardsResponseDto> {
    const { supabase, userId, flashcards } = params;

    const aiSources = new Set<FlashcardDraftCommand["source"]>(["ai-full", "ai-edited"]);

    const requiredGenerationIds = Array.from(
      new Set(
        flashcards.filter((fc) => aiSources.has(fc.source) && fc.generation_id).map((fc) => fc.generation_id as string)
      )
    );
    ...
    if (requiredGenerationIds.length > 0) {
      ...
      if (genError) {
        throw new FlashcardServiceError("Failed to verify generation ownership", "GENERATION_NOT_FOUND", genError);
      }

      const foundIds = new Set((generations ?? []).map((g) => g.id as string));
      const missing = requiredGenerationIds.filter((id) => !foundIds.has(id));
      if (missing.length > 0) {
        throw new FlashcardServiceError("Generation not found", "GENERATION_NOT_FOUND");
      }
    }
    ...
    if (error || !data || data.length === 0) {
      throw new FlashcardServiceError("Failed to create flashcards", "FLASHCARDS_PERSISTENCE_FAILED", error);
    }
  ...
  async listFlashcards(params: ListFlashcardsParams): Promise<FlashcardListResponseDto> {
    ...
    const { data, error, count } = await builder.order(sortColumn, { ascending }).range(offset, offset + limit - 1);
  ...
  async updateFlashcard(params: {
    ...
    if (!hasAnyField) {
      throw new FlashcardServiceError("No fields to update", "INVALID_UPDATE_PAYLOAD");
    }
    ...
    if (currentError) {
      throw new FlashcardServiceError("Failed to fetch flashcard", "FLASHCARDS_PERSISTENCE_FAILED", currentError);
    }
    ...
    const aiSources = new Set<FlashcardDetailDto["source"]>(["ai-full", "ai-edited"]);

    const targetSource = command.source ?? (current.source as FlashcardDetailDto["source"]);
    let targetGenerationId: string | null = current.generation_id as string | null;
    ...
    if (command.source !== undefined) {
      if (command.source === "manual") {
        ...
        if (typeof command.generation_id !== "string") {
          throw new FlashcardServiceError("generation_id is required for AI sources", "INVALID_GENERATION_LINK");
        }
      ...
      if (currentIsManual) {
        throw new FlashcardServiceError("Cannot set generation_id when source is manual", "INVALID_GENERATION_LINK");
      }
      targetGenerationId = command.generation_id;
    }
    ...
    if (updateError) {
      throw new FlashcardServiceError("Failed to update flashcard", "FLASHCARDS_PERSISTENCE_FAILED", updateError);
    }
  ...
  async deleteFlashcard(params: { supabase: SupabaseClient; userId: string; flashcardId: string }): Promise<boolean> {
    const { supabase, userId, flashcardId } = params;
    ...
    if (error) {
      throw new FlashcardServiceError("Failed to delete flashcard", "FLASHCARDS_PERSISTENCE_FAILED", error);
    }
